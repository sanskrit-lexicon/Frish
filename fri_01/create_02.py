...
def adjust_pc_unused(pc):
 # pc is either X  or X-Y where X,Y are digit-sequences
 # change to 0-filled digit sequences
 m = re.search(r'^([0-9]+)$',pc)
 if m:
  x = int(m.group(1))
  xnew = '%03d' % x
  return xnew
 # the other form
 m = re.search(r'^([0-9]+)-([0-9]+)$',pc)
 if m:
  x = int(m.group(1))
  xnew = '%03d' % x
  y = int(m.group(2))
...
def make_02_meta(oldmeta):
 ...
 k2 = m.group(4)
 pc1 = adjust_pc(pc)
 newmeta = '<L>%s<pc>%s<k1>%s<k2>%s' %(L,pc1,k1,k2)
 return newmeta
...
def make_newrecs(oldrecs):
 newrecs = []
 for irec,oldrec in enumerate(oldrecs):
  newrec = make_02_rec(oldrec)
  newrecs.append(newrec)
 return newrecs

if __name__ == "__main__":
 filein = sys.argv[1] # fri_01.txt
 fileout = sys.argv[2] # fri_02.txt
 # read the lines from filein
 lines = read_lines(filein)
 oldrecs = list(generate_oldrecs(lines))
 print(len(oldrecs),'records')
 newrecs = make_newrecs(oldrecs)
 #newrecs = [make_02_rec(rec) for rec in oldrecs]
